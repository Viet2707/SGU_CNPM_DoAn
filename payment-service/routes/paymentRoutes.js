...
if (!stripeKey) {
  console.warn('⚠️  STRIPE_SECRET_KEY missing — payment routes disabled (dev fallback)');
  router.post('/customer', verifyToken, allowRoles('customer'), (_req, res) =>
    res.status(503).json({ message: 'Payment disabled (missing STRIPE_SECRET_KEY)' })
  );
  router.post('/create-payment-intent', verifyToken, allowRoles('customer'), (_req, res) =>
    res.status(503).json({ message: 'Payment disabled (missing STRIPE_SECRET_KEY)' })
  );
  router.get('/verify-payment/:paymentIntentId', verifyToken, allowRoles('customer'), (_req, res) =>
    res.status(503).json({ message: 'Payment disabled (missing STRIPE_SECRET_KEY)' })
  );
  router.post('/update/:paymentIntentId', verifyToken, allowRoles('customer'), (_req, res) =>
    res.status(503).json({ message: 'Payment disabled (missing STRIPE_SECRET_KEY)' })
  );
